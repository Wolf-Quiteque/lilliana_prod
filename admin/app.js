...
async function uploadMedia(event) { event.preventDefault(); const file = event.currentTarget.elements.file.files[0]; if (!file) return; try { const response = await fetch("/api/media", { method:"POST", headers:{ "Content-Type": file.type, "X-File-Name": encodeURIComponent(file.name) }, body:file }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.error || "Upload failed."); state.media.unshift(body); state.message = "Image uploaded to R2. Copy its URL into a portfolio item."; state.error = ""; render(); } catch (error) { state.error = error.message; render(); } }
